...
10. **v10** — En mobile, `.message-card` había quedado en `top: 30%` mientras `.garden-name` estaba en `top: 40%` (divergencia heredada de cuando el mensaje se cortaba contra el suelo en v7). El espacio liberado en v9 ya alcanza para que ambos compartan el mismo `top: 40%` sin que el suelo tape ninguna línea del mensaje — quedaron igualados, como en desktop (que ya compartían `top: 26%` desde antes).
11. **v11** (actual) — Validación del formulario (`app.js` + `pattern` en el input de `index.html`): el campo de nombre filtra en vivo (evento `input`) cualquier carácter que no sea letra (incluye acentos y ñ), espacio, apóstrofo o guion — números y símbolos se eliminan mientras se escriben, no se avisan después. El signo zodiacal ya era `required` desde antes (sin cambios). El color ya era opcional desde antes — tiene un valor por defecto y nunca tuvo `required`, así que no hacía falta tocar nada ahí.
